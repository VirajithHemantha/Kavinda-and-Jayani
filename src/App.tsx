...
/**
 * Premium Black & White with Gold Wedding Invitation Theme
 * Names: Kavinda & Jayani
 * Background: White
 * Accents: Gold
 */
...
const flowerArchImage = "/upper.jpg";
const brideGroomImage = "/back.jpg";
...
function FloatingPetals({ disabled = false, vibrant = false }: { disabled?: boolean; vibrant?: boolean }) {
  ...
  useEffect(() => {
    ...
    const vibrantColors = ["#D4AF37", "#FFFFFF", "#111111", "#FFD700", "#F5EEDB"];
...
function CountdownTimer() {
  const targetDate = new Date("August 7, 2026 09:30:00").getTime();
...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyzM5u5E7EiGZd1YiKKjt4L4-5kVje_JPzeFqFYxmCGnG0F98Yxpt0uERA9_OJZj7fi/exec";
...
function AdminPage() {
  ...
  const handleCopyMessage = () => {
    // If the prefix is "Dear", we just use "Dear name", otherwise "Dear prefix name"
    const prefixStr = prefix === "Dear" ? "" : (prefix ? prefix + ' ' : '');
    const message = `Dear ${prefixStr}${name} ❤️\n\nWith joyful hearts, we warmly invite you to celebrate one of the most special days of our lives as we begin our journey together.\n\nPlease view our wedding invitation and all the event details through the link below 🌐:\n\n${generatedLink}\n\nYour presence would truly mean the world to us, and we would be honored to celebrate this beautiful moment together.\n\nWith love,\n❤️ Kavinda & Jayani`;
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. Sanjaya" value={name} onChange={(e) => setName(e.target.value)} className="w-full border-b border-theme-200 py-2 focus:outline-none focus:border-theme-500 bg-transparent" />
          </div>
        ...
        {generatedLink && (
          <div ref={resultRef} className="mt-6 space-y-6 bg-theme-50 p-6 rounded-xl">
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-stone-500 uppercase tracking-widest">Generated Link Format:</label>
              <p className="text-xs text-stone-700 break-all bg-white p-3 rounded border border-theme-200">{generatedLink}</p>
            </div>
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-stone-500 uppercase tracking-widest">Generated Invitation Message Template:</label>
              <div className="text-xs text-stone-700 whitespace-pre-wrap bg-white p-4 rounded border border-theme-200">
                {`Dear ${prefix === "Dear" ? "" : (prefix ? prefix + ' ' : '')}${name} ❤️\n\nWith joyful hearts, we warmly invite you to celebrate one of the most special days of our lives as we begin our journey together.\n\nPlease view our wedding invitation and all the event details through the link below 🌐:\n\n${generatedLink}\n\nYour presence would truly mean the world to us, and we would be honored to celebrate this beautiful moment together.\n\nWith love,\n❤️ Kavinda & Jayani`}
              </div>
            </div>
            <div className="space-y-3 pt-2">
              <button onClick={handleCopyLink} className="w-full border border-theme-300 text-theme-800 py-2 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-theme-100 transition-colors">Copy Link Only</button>
              <button onClick={handleCopyMessage} className="w-full border border-theme-800 bg-theme-800 text-white py-2 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-theme-900 transition-colors">Copy Full Message</button>
            </div>
          </div>
        )}
      </div>
        ...
        {!isOpened ? (
          ...
          >
            ...
            <motion.div
              ...
              className="absolute -top-[10%] -left-[10%] w-[60%] aspect-square bg-gradient-to-br from-theme-400/30 to-white/10 blur-[120px] rounded-full"
            />
            <motion.div
              animate={{
                scale: [1.2, 1, 1.2],
              ...
              className="absolute -bottom-[15%] -right-[15%] w-[70%] aspect-square bg-gradient-to-tl from-theme-500/40 to-theme-200/20 blur-[140px] rounded-full"
            />
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-theme-400/10 via-transparent to-white/10" />
            ...
            >
              <div className="absolute -inset-4 bg-gradient-to-r from-theme-400 via-white to-theme-600 blur-2xl opacity-40 animate-pulse" />
              <motion.button
                whileHover={{ scale: 1.05, backgroundColor: "rgba(255, 255, 255, 1)" }}
                whileTap={{ scale: 0.95 }}
                onClick={openInvitation}
                className="group relative flex flex-col items-center gap-4 bg-white/95 backdrop-blur-md border border-white/60 px-14 py-6 rounded-full text-stone-800 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.12)] transition-all duration-500 hover:shadow-theme-400/30"
              >
                ...
                <div className="w-10 h-[1.5px] bg-gradient-to-r from-theme-400 via-white to-theme-400 group-hover:w-20 transition-all duration-500" />
            ...
            <div className="absolute top-12 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
              <p className="font-cinzel text-[11px] tracking-[0.8em] font-bold uppercase flex flex-col items-center gap-3">
                <span className="bg-gradient-to-r from-yellow-400 via-yellow-200 to-yellow-500 bg-clip-text text-transparent opacity-80 ml-[0.8em]">K & J</span>
            ...
            <div className="absolute top-0 left-0 w-44 h-44 opacity-40 pointer-events-none overflow-hidden z-10">
              <img src={flowerCornerImage} className="w-full h-full object-contain -rotate-90 sepia-[1] hue-rotate-[10deg] saturate-[3] brightness-[1.1] contrast-[1.2]" alt="" />
            </div>
            <div className="absolute top-0 right-0 w-44 h-44 opacity-40 pointer-events-none overflow-hidden z-10">
              <img src={flowerCornerImage} className="w-full h-full object-contain sepia-[1] hue-rotate-[10deg] saturate-[3] brightness-[1.1] contrast-[1.2]" alt="" />
            </div>
          </motion.div>
        ) : (
          <motion.div
          ...
          >
            ...
            <section className="min-h-[100dvh] w-full flex items-center justify-center p-4 md:p-12 relative overflow-hidden bg-[#FFFFFF]">
              ...
              <motion.div
                ...
                className="relative z-10 w-full max-w-[420px] min-h-[500px] h-[85vh] md:h-[80vh] bg-[#ffffff] rounded-t-full shadow-[0_20px_60px_-15px_rgba(212,175,55,0.15)] border border-theme-500 flex flex-col items-center overflow-hidden p-6 pt-12 md:p-10 md:pt-16"
              >
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] h-auto z-0 pointer-events-none -translate-y-4 md:-translate-y-8">
                  <motion.img
                    ...
                    style={{ WebkitMaskImage: 'linear-gradient(to bottom, black 35%, transparent 55%)', maskImage: 'linear-gradient(to bottom, black 35%, transparent 55%)' }}
                  />
                </div>
                ...
                <div className="flex flex-col items-center text-center space-y-4 flex-1 w-full relative z-10">
                  ...
                  <div className="space-y-0 py-4 flex-1 flex flex-col justify-center">
                    <motion.h1
                      ...
                      className="font-playball text-[3rem] sm:text-[3.5rem] md:text-[5rem] text-stone-900 leading-[1.1] drop-shadow-sm"
                    >
                      Kavinda
                    </motion.h1>
                    <motion.div
                    ...
                    >
                      {hasGuest ? (
                        <div className="flex flex-col items-center space-y-2">
                          <span className="text-[10px] md:text-[13px] text-stone-900 font-cinzel px-5 py-2">
                            We cordially invite
                          </span>
                          <span className="text-xl md:text-2xl font-playball font-bold text-theme-600">
                            {guestPrefix} {guestName}
                          </span>
                        </div>
                      ) : (
                        <span className="inline-block text-[10px] md:text-[13px] uppercase tracking-[0.6em] md:tracking-[0.8em] text-stone-900 font-bold px-5 py-2 rounded-full bg-theme-400/20 border border-theme-400/30">
                          Please join us
                        </span>
                      )}
                    </motion.div>
                  ...
                  >
                    ...
                    <div className="font-cinzel space-y-1">
                      <p className="text-sm md:text-base text-stone-900 tracking-[0.2em] md:tracking-[0.3em] font-bold">07.08.2026</p>
                      <p className="text-[8px] md:text-[9px] text-theme-600 tracking-[0.2em] uppercase font-bold">AVINKA GRAND LUXURY BANQUETS</p>
                    </div>
                  </motion.div>
                </div>
              </motion.div>
              ...
              >
                <span className="text-[8px] md:text-[9px] uppercase tracking-widest text-stone-500 font-bold group-hover:text-theme-500 transition-colors">Begin</span>
            ...
            <section className="cv-auto py-24 md:py-32 w-full flex flex-col items-center px-4 relative">
              <div className="section-floral-overlay absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden z-0">
                ...
                <InviteImage src={flowerImage} className="absolute -left-12 -top-12 w-[220px] md:w-[380px] h-auto opacity-70 object-contain -rotate-[105deg] sepia-[1] hue-rotate-[10deg] saturate-[3] brightness-[1.1] contrast-[1.2]" alt="" />
                <InviteImage src={flowerImage} className="absolute -right-12 -top-12 w-[220px] md:w-[380px] h-auto opacity-70 object-contain -rotate-[15deg] sepia-[1] hue-rotate-[10deg] saturate-[3] brightness-[1.1] contrast-[1.2]" alt="" />
                <InviteImage src={flowerImage} className="absolute -left-12 -bottom-12 w-[220px] md:w-[380px] h-auto opacity-70 object-contain rotate-[165deg] sepia-[1] hue-rotate-[10deg] saturate-[3] brightness-[1.1] contrast-[1.2]" alt="" />
                <InviteImage src={flowerImage} className="absolute -right-12 -bottom-12 w-[220px] md:w-[380px] h-auto opacity-70 object-contain rotate-[75deg] sepia-[1] hue-rotate-[10deg] saturate-[3] brightness-[1.1] contrast-[1.2]" alt="" />
              </div>
              ...
              <div className="max-w-[1000px] w-full flex flex-col items-center text-center relative z-10">
                ...
                >
                  <div className="w-[2px] h-16 md:h-24 bg-gradient-to-b from-transparent via-theme-400 to-transparent mb-8 md:mb-12 shadow-[0_0_10px_rgba(212,175,55,0.3)]" />
                  <p className="text-stone-900 text-[10px] md:text-[14px] tracking-[0.6em] md:tracking-[0.8em] uppercase font-bold text-center leading-relaxed max-w-2xl px-4">
                    <span className="text-theme-600 block text-[8px] md:text-[10px] mb-3 md:mb-4 tracking-[0.8em]">Wedding Celebration</span>
                    You are cordially invited to<br className="hidden md:block" />
                    <span className="text-theme-600">celebrate the union of</span>
                  </p>
                  <div className="w-16 h-[2px] bg-gradient-to-r from-transparent via-theme-400 to-transparent mt-8 md:mt-10 shadow-[0_0_10px_rgba(212,175,55,0.3)]" />
                </motion.div>
                  ...
                  >
                    ...
                    <div className="relative z-10 space-y-4 py-8 md:py-12">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs md:text-sm font-cinzel text-stone-600 tracking-wide leading-relaxed">Mr. Sisi Kumara<br />& Mrs. Manel Kanthi</p>
                      </div>
                      <h3 className="text-5xl md:text-7xl font-playball text-theme-800 group-hover:scale-110 transition-transform duration-700 pt-6 drop-shadow-sm">Kavinda</h3>
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="relative z-10 space-y-4 py-8 md:py-12">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs md:text-sm font-cinzel text-stone-600 tracking-wide leading-relaxed">Mr. Nimal Jayasiri (late)<br />& Mrs. Anusha Krishanthi</p>
                      </div>
                      <h3 className="text-5xl md:text-7xl font-playball text-theme-800 group-hover:scale-110 transition-transform duration-700 pt-6 drop-shadow-sm">Jayani</h3>
                    </div>
                  </motion.div>
                </div>
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-32 text-center w-full max-w-4xl px-4">
                    <div className="flex flex-col items-center flex-1">
                      ...
                      <p className="font-cinzel text-xl md:text-3xl text-theme-900 tracking-widest font-bold whitespace-nowrap">07.08.2026</p>
                  ...
                  >
                    ...
                    <div className="space-y-4 flex flex-col items-center">
                      ...
                      <div className="flex items-center gap-6 mt-6">
                        ...
                        <div className="flex flex-col items-center">
                          <p className="font-cinzel text-xl md:text-2xl text-theme-700 font-bold tracking-[0.2em]">09:30 AM</p>
                          <p className="text-[8px] md:text-[9px] uppercase tracking-[0.3em] text-theme-400 font-bold mt-1">Starts</p>
                        </div>
                ...
                <div className="grid md:grid-cols-2 gap-16 md:gap-24 items-center">
                  ...
                  >
                    ...
                    <div className="space-y-6 pt-4 relative">
                      ...
                      <div className="pl-8 space-y-4">
                        <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#111111] shadow-sm border border-theme-500 absolute -left-5 top-0">
                          <MapPin className="w-4 h-4 text-theme-400" />
                        </div>
                        <p className="text-lg md:text-xl text-stone-800 font-cinzel font-medium leading-relaxed tracking-wide">
                          Homagama
                        </p>
                      </div>

                      <div className="pl-8 space-y-4 pt-4 text-stone-600 text-sm md:text-base tracking-wide font-light leading-relaxed">
                    ...
                    <div className="pt-8 w-full md:w-auto">
                      <button
                        onClick={() => window.open('https://maps.app.goo.gl/WKMGgfwA9SFRS6R86', '_blank')}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 w-full h-full scale-[1.2] group-hover:scale-[1.15] transition-transform duration-[2s]">
                      <iframe
                        src="https://maps.google.com/maps?q=AVINKA%20GRAND%20LUXURY%20BANQUETS,%20Homagama&t=&z=14&ie=UTF8&iwloc=&output=embed"
                        width="100%"
                        height="100%"
                        style={{ border: 0 }}
                        allowFullScreen
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-white via-white/80 to-transparent h-32 pointer-events-none z-10 flex items-end justify-center pb-6">
                      <p className="text-[8px] uppercase tracking-widest text-theme-400 font-bold bg-[#111111] px-5 py-2 rounded-full shadow-sm backdrop-blur-md inline-flex items-center gap-2 border border-theme-500/50">
            ...
            {/* RSVP Section */}
            <section className="cv-auto py-24 md:py-36 bg-[#F9FAFB] text-stone-900 relative overflow-hidden flex flex-col items-center">
              {/* Opulent dark background */}
              <div className="absolute inset-0 opacity-10 paper-grain pointer-events-none" />
              <div className="absolute top-0 right-0 w-[60vw] h-[60vw] max-w-[800px] bg-theme-200 blur-[150px] rounded-full opacity-20 pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-[60vw] h-[60vw] max-w-[800px] bg-theme-300 blur-[150px] rounded-full opacity-20 pointer-events-none" />
              ...
              <div className="container mx-auto px-4 max-w-2xl text-center relative z-10 w-full">
                ...
                >
                  <p className="text-[10px] md:text-[12px] uppercase tracking-[0.5em] md:tracking-[0.8em] text-stone-400 font-bold mb-6">Will You Join Us?</p>
                  ...
                  {/* Premium RSVP Form */}
                  <div className="w-full bg-[#111111] p-6 sm:p-8 md:p-12 rounded-[2rem] border border-theme-500/30 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] relative overflow-hidden">
                    {/* Inner gold decorative border */}
                    <div className="absolute inset-3 border border-theme-400/20 rounded-[1.5rem] pointer-events-none" />
                    
                    <form className="space-y-8 text-left relative z-10" onSubmit={(e) => { e.preventDefault(); handleSubmit("rsvp", rsvpData); }}>
            ...
            <div className="relative bg-[#FFFFFF]">
              ...
              <section className="cv-auto py-24 md:py-36 relative flex flex-col items-center overflow-hidden">
                ...
                <div className="container mx-auto px-4 max-w-4xl text-center relative z-10 w-full">
                  ...
                  >
                    ...
                    <h2 className="font-playball text-[3.5rem] sm:text-[4rem] md:text-[5.5rem] text-theme-900 mb-6 drop-shadow-sm leading-none">Best Wishes</h2>
                    <div className="h-px w-24 bg-gradient-to-r from-transparent via-theme-400 to-transparent mb-8" />

                    <p className="text-stone-600 text-sm md:text-lg leading-relaxed max-w-xl mx-auto mb-16 font-light tracking-wide px-4">
                    ...
                    <div className="w-full max-w-2xl mx-auto bg-white p-6 sm:p-8 md:p-14 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] border border-theme-200/50 rounded-tr-[4rem] rounded-bl-[4rem] relative group">
                    ...
                    <div className="mt-32 md:mt-48 space-y-6 flex flex-col items-center relative w-full">
                      ...
                      <p className="text-[9px] md:text-[11px] uppercase tracking-[0.8em] text-stone-900 font-bold relative z-10 bg-[#F9FAFB] px-6 py-2 rounded-full border border-theme-200/50 shadow-sm">With Love</p>
                      <h3 className="font-playball text-[3.2rem] sm:text-6xl md:text-8xl text-theme-900 relative z-10 drop-shadow-sm px-4 pt-4 leading-none">Kavinda & Jayani</h3>
              ...
              <footer className="py-20 border-t border-theme-200/30 text-center relative z-10 space-y-6">
                <p className="text-[9px] md:text-[11px] uppercase tracking-[0.5em] text-stone-500 font-bold">
                  © 2026 Kavinda & Jayani. All rights reserved.
                </p>
                ...
                <div className="flex flex-col items-center gap-2 pt-8">
                  ...
                  >
                    ...
                    <span className="font-playball text-2xl md:text-3xl text-theme-900 group-hover:scale-110 transition-transform duration-500">
                      invitemint
                    </span>
                    <span className="text-[9px] md:text-[11px] tracking-[0.2em] text-stone-400 font-medium mt-1">
                      077 572 2949 / 075 658 6847
                    </span>
